...
map.on('click', 'yoga-boundary', (event) => {
    // Get the clicked feature's properties
    const properties = event.features[0].properties;

    // Use a popup to display the feature's attributes
    new mapboxgl.Popup()
        .setLngLat(event.lngLat) // Set the popup at the clicked location
        .setHTML(`
            <h4>Yoga Studio Info</h4>
            <p><strong>Boundary Name:</strong> ${properties.name || 'N/A'}</p>
            <p><strong>Other Attribute:</strong> ${properties.attribute || 'N/A'}</p>
        `) // Customize this HTML with your feature attributes
        .addTo(map); // Add the popup to the map
});

// Change the cursor to a pointer when hovering over the boundary
map.on('mouseenter', 'yoga-boundary', () => {
    map.getCanvas().style.cursor = 'pointer';
});

// Reset the cursor when it leaves the boundary
map.on('mouseleave', 'yoga-boundary', () => {
